...
def run_model():
    ...
    valid_models = ["linear_regression", "decision_tree", "random_forest",
                    "svm", "knn", "logistic_regression", "gradient_boosting", "xgboost"]
    ...
    if (result_array[1] == "prediction"):
        results, y_test, y_test_pred = train_predict_regression(
            data_csv, model_name, target=result_array[2])
        un_results, un_y_test, un_y_test_pred = regression_standard(
            data_csv, model_name, target=result_array[2])
    elif (result_array[1] == "classification"):
        results, y_test, y_test_pred = train_predict_classification(
            data_csv, model_name, target=result_array[2])
        un_results, un_y_test, un_y_test_pred = classification_standard(
            data_csv, model_name, target=result_array[2])
    else:
        pass
